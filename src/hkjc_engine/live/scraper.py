        ...
        try:
            if "info.cld.hkjc.com/graphql" in response.url and response.status == 200:
                ...
                if 'application/json' in content_type:
                    ...
                    # --- 1. INTERCEPT POOL LIQUIDITY PAYLOAD ---
                    if 'poolInvs' in first_meeting:
                        # Save the raw liquidity payload to a distinct Redis key
                        r_cache.setex(
                            f"live_investment_raw:{self.venue}:{self.race_no}", 
                            10000, 
                            json.dumps(data)
                        )
                        captured_invs = [p.get('oddsType') for p in first_meeting['poolInvs'] if p.get('oddsType')]
                        if captured_invs:
                            time_str = datetime.datetime.now().strftime('%H:%M:%S')
                            tag = " [POST-SELL]" if self.post_stop_sell_mode else ""
                            print(f"[{time_str}] API Caught Liquidity{tag}: {', '.join(set(captured_invs))} for R{self.race_no}")

                    # --- 2. INTERCEPT RUNNERS METADATA ---
                    if 'races' in first_meeting:
                        races = first_meeting.get('races', [])
                        if races and 'runners' in races[0]:
                            r_cache.setex(f"live_odds_raw:{self.venue}:{self.race_no}:runners", 9999, json.dumps(data))
                    
                    # --- 3. INTERCEPT ODDS MATRIX PAYLOAD ---
                    if 'pmPools' in first_meeting:
                        pools = first_meeting.get('pmPools', [])
                        saved_pools = []
                        ...
                        for p in pools:
                            ...
                            if status in ('STOP_SELL', 'STOPSELL'):
                                if not self.post_stop_sell_mode:
                                    self.stop_sell_seen_at = datetime.datetime.now()
                                    self.post_stop_sell_mode = True
                                    r_cache.setex(f"race_status:{self.venue}:{self.race_no}", 600, 'STOP_SELL')
                                    r_cache.setex(f"stop_sell_time:{self.venue}:{self.race_no}", 86400, self.stop_sell_seen_at.isoformat())
                                    print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] API DETECTED 'STOP_SELL'. Entering {POST_STOP_SELL_POLL_SECONDS}s late-money capture window.")
                            elif status and not self.post_stop_sell_mode:
                                r_cache.setex(f"race_status:{self.venue}:{self.race_no}", 300, status)
                            
                            if o_type and int(self.race_no) in races:
                                isolated_payload = {
                                    "data": {"raceMeetings": [{"pmPools": [p]}]}
                                }
                                r_cache.setex(f"live_odds_raw:{self.venue}:{self.race_no}:odds:{o_type}", 10000, json.dumps(isolated_payload))
                                saved_pools.append(o_type)
                        ...
                        if saved_pools:
                            time_str = datetime.datetime.now().strftime('%H:%M:%S')
                            tag = " [POST-SELL]" if self.post_stop_sell_mode else ""
                            print(f"[{time_str}] API Caught Odds{tag}: {', '.join(set(saved_pools))} for R{self.race_no}")
        except Exception:
            pass

    async def extract_metadata(self, page):
        # [Unchanged: Metadata extraction logic]
        try:
            await page.wait_for_selector(".meeting-info-content-text", timeout=10000)
            info_text = await page.locator(".meeting-info-content-text").inner_text()
            dist_match = re.search(r'(\d+)\s*[mM]', info_text)
            ...
            rail = "AWT" if "ALL WEATHER" in info_text.upper() or "AWT" in info_text.upper() else "A"
            rail_match = re.search(r'"(.+)"\s+Course', info_text)
            if rail_match and rail == "A": rail = rail_match.group(1)

            parts = [p.strip() for p in info_text.split(',')]
            race_time = parts[2] if len(parts) > 2 else "Unknown"
            if ":" not in race_time:
                time_match = re.search(r'(\d{1,2}:\d{2})', info_text)
                if time_match: race_time = time_match.group(1)

            metadata = {
                "venue": self.venue, "race_no": self.race_no, "class": race_class,
                "distance": distance, "rail": rail, "time": race_time,
                "scraped_at": datetime.datetime.now().strftime('%H:%M:%S')
            }
            r_cache.set(f"live_race_metadata:{self.venue}:{self.race_no}", json.dumps(metadata))
            return metadata
                ...
                while True:
                    if self.post_stop_sell_mode and self.stop_sell_seen_at is not None:
                        ...
                        if elapsed >= POST_STOP_SELL_POLL_SECONDS:
                            ...
                            print(f"Late-money window complete. Wrote CLOSED. Shutting down.")
                            break
